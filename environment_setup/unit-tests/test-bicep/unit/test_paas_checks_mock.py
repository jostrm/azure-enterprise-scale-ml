...
class TestPaasChecksMocked(unittest.TestCase):
    ...
    @mock.patch("domain.paas.cli.az_json", return_value={"name": "acr1"})
    def test_acr_reachable_when_private_and_show_ok(self, _json, _dns) -> None:
        self.assertTrue(paas.acr_reachable("acr1"))

    @mock.patch("domain.paas.cli.resolves_to_private_ip", return_value=False)
    @mock.patch("domain.paas.cli.az_json", return_value=None)
