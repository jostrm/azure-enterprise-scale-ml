...
from base.cli import CmdResult
from domain import factory, governance, iac, paas, project


class TestWhatIfClean(unittest.TestCase):
    @mock.patch("domain.iac.cli.az", return_value=CmdResult(0, '{"changes":[{"changeType":"NoChange"}]}', ""))
    def test_no_drift(self, _az) -> None:
        self.assertTrue(iac.what_if_clean("rg", Path("t.bicep"), {"a": "1"}))

    @mock.patch("domain.iac.cli.az", return_value=CmdResult(0, '{"changes":[{"changeType":"Modify"}]}', ""))
...
class TestIdentityRoles(unittest.TestCase):
    @mock.patch("domain.governance.cli.az_json", return_value=[{"roleDefinitionName": "Reader"}, {"roleDefinitionName": "Reader"}])
    def test_dedup_sorted(self, _json) -> None:
        self.assertEqual(["Reader"], governance.identity_roles("pid"))

    @mock.patch("domain.governance.cli.az_json", return_value=None)
...
class TestFoundryConnections(unittest.TestCase):
    @mock.patch("domain.paas.cli.resolves_to_private_ip", return_value=True)
    @mock.patch("domain.paas.cli.az_json", return_value=[{"properties": {"target": "srch1.search.windows.net"}}])
    def test_reaches_search(self, _json, _dns) -> None:
        self.assertTrue(paas.foundry_can_reach_search("f1", "srch1"))

    @mock.patch("domain.paas.cli.resolves_to_private_ip", return_value=True)
    @mock.patch("domain.paas.cli.az_json", return_value=[])
...
class TestCleanupIdempotent(unittest.TestCase):
    @mock.patch("domain.factory.cli.az_json", return_value=None)
    ...
    @mock.patch("domain.project.cli.az", return_value=CmdResult(0, "", ""))
    @mock.patch("domain.project.cli.az_json", return_value={"name": "rg"})
