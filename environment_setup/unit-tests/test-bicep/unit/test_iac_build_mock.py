...
class TestIacBuildMocked(unittest.TestCase):
    ...
    @mock.patch("domain.iac.cli.az", return_value=CmdResult(0, "{}", ""))
    def test_build_ok_when_compiler_succeeds(self, _az) -> None:
        self.assertTrue(iac.build_ok(Path("any.bicep")))

    @mock.patch(
        "domain.iac.cli.az", return_value=CmdResult(1, "", "Error BCP000")
    )
    ...
    @mock.patch("domain.iac.cli.az", return_value=CmdResult(0, "", "warn"))
