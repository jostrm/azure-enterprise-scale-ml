...
@description('Optional. Resource ID of the route table for AKS2 subnet. Used for User Defined Routing (UDR) with private AKS.')
param aks2RouteTableId string = ''
@description('Specifies the outbound (egress) routing method for AKS2 cluster')
@allowed([
  'loadBalancer'
  'userDefinedRouting'
])
...
module aks2Snt '../modules/subnetWithNsg.bicep' = if (!empty(aks2SubnetCidr) && !sntAks002Exists) {
  ...
  params: {
    ...
    routeTableId: aks2RouteTableIdToUse // Use provided route table or auto-created one for UDR
    centralDnsZoneByPolicyInHub:centralDnsZoneByPolicyInHub
  }
  dependsOn: [
...
output aksSubnetId string = !sntAksExists ? aksSnt!.outputs.subnetId : resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, !empty(subnetProjAKS) ? subnetProjAKS : 'snt-${projectName}-aks')
output aks2SubnetId string = (!sntAks002Exists && !empty(aks2SubnetCidr)) ? aks2Snt!.outputs.subnetId : (sntAks002Exists ? resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, !empty(subnetProjAKS2) ? subnetProjAKS2 : 'snt-${projectName}-aks-002') : '')
output genaiSubnetId string = !sntGenaiExists ? genaiSnt!.outputs.subnetId : resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, !empty(subnetProjGenAI) ? subnetProjGenAI : 'snt-${projectName}-genai')
output acaSubnetId string = !sntAcaExists ? acaSnt!.outputs.subnetId : resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, !empty(subnetProjACA) ? subnetProjACA : 'snt-${projectName}-aca')
output aca2SubnetId string = (!sntAca002Exists && !empty(aca2SubnetCidr)) ? acaSnt2!.outputs.subnetId : (sntAca002Exists ? resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull,!empty(subnetProjACA2) ? subnetProjACA2 :  'snt-${projectName}-aca-002') : '')
output dbxPubSubnetName string = empty(dbxPubSubnetCidr) ? '' : (!empty(subnetProjDatabricksPublic) ? subnetProjDatabricksPublic : 'snt-${projectName}-dbxpub')
output dbxPrivSubnetName string = empty(dbxPrivSubnetCidr) ? '' : (!empty(subnetProjDatabricksPrivate) ? subnetProjDatabricksPrivate : 'snt-${projectName}-dbxpriv')
