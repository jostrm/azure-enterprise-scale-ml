...
output aksSubnetId string = !sntAksExists ? aksSnt!.outputs.subnetId : resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, !empty(subnetProjAKS) ? replace(subnetProjAKS, '<xxx>', projectNumber) : 'snt-${projectName}-aks')
output aks2SubnetId string = (!sntAks002Exists && !empty(aks2SubnetCidr)) ? aks2Snt!.outputs.subnetId : (sntAks002Exists ? resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, !empty(subnetProjAKS2) ? replace(subnetProjAKS2, '<xxx>', projectNumber) : 'snt-${projectName}-aks-002') : '')
