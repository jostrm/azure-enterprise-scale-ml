...
output aks2SubnetId string = (!sntAks002Exists && !empty(aks2SubnetCidr)) ? aks2Snt!.outputs.subnetId : (sntAks002Exists ? resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, 'snt-${projectName}-aks-002') : '')
...
output aca2SubnetId string = (!sntAca002Exists && !empty(aca2SubnetCidr)) ? acaSnt2!.outputs.subnetId : (sntAca002Exists ? resourceId(subscription().subscriptionId, vnetResourceGroup, 'Microsoft.Network/virtualNetworks/subnets', vnetNameFull, 'snt-${projectName}-aca-002') : '')
