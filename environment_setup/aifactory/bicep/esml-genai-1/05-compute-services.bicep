...
// Per-environment AKS node SKU (vmSize) and SKU tier passed from the pipeline (Dev vs Stage/Prod)
@description('AKS node vmSize for dev. Empty = template default. Takes precedence over aks_dev_sku_override.')
param skuAksDev string = ''
@description('AKS node vmSize for stage/prod (test/prod). Empty = template default. Takes precedence over aks_test_prod_sku_override.')
param skuAksStageProd string = ''
@description('AKS SKU tier for dev. Empty = fall back to aksSkuTier.')
param skuTierAksDev string = ''
@description('AKS SKU tier for stage/prod (test/prod). Empty = fall back to aksSkuTier.')
param skuTierAksStageProd string = ''
...
// Precedence: per-env pipeline SKU (skuAksDev/skuAksStageProd) > legacy override > template default
var aks_dev_sku_param = !empty(skuAksDev) ? skuAksDev : (!empty(aks_dev_sku_override) ? aks_dev_sku_override : aks_dev_defaults[0])
