...
/* This (no AVM module) is better, since using AI Factory buildRoleAssignments.bicep
┌─────────────────────────────────────────────────────────┐
│  This has not POTENTIAL OVER-ASSIGNMENT ISSUE           │
│  ┌─────────────────────────────────────────────────────┐│
│  │ From: Deploy-Your-AI-Application-In-Production repo:││
│  │  aiFoundry2025rbac.bicep from AVM corp assigns:     ││
│  │  Users → Contributor + User roles (MORE access)     ││
│  │  SP    → User roles only      (LESS access)         ││
│  └─────────────────────────────────────────────────────┘│
│  ┌─────────────────────────────────────────────────────┐│
│  │ From: Enterprise scale AI Factory (jostrm) repo:    ││
│  │  buildRoleAssignments.bicep in AIFactory assigns:   ││
...
└─────────────────────────────────────────────────────────┘
┌─────────────────────────────────────────────────────────┐
│  The AIFactoru Uses AVM roleAssignmentType[] parameter  │
│  ┌─────────────────────────────────────────────────────┐│
│  │  Generic Role Assignment Framework                  ││
│  │  • Takes any roleAssignmentType[] array             ││
│  │  │  └─ principalId                                  ││
│  │  │  └─ roleDefinitionIdOrName                       ││
│  │  │  └─ principalType                                ││
│  │  │  └─ condition (optional)                         ││
│  │  │  └─ description (optional)                       ││
│  │  └─ Deploys via: Microsoft.Authorization/           ││
│  │     roleAssignments@2022-04-01                      ││
│  └─────────────────────────────────────────────────────┘│
│  📝 Flexible - accepts any role configuration           │
└─────────────────────────────────────────────────────────┘
*/
