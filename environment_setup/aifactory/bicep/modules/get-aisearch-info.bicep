...
// Guard: ARM validates resourceId() even in the false branch of a ternary, so we must
// ensure aiSearchName is never empty to avoid a malformed resource identifier.
var safeName = !empty(aiSearchName) ? aiSearchName : 'placeholder'

// Outputs - handle the conditional logic here
@description('Principal ID of AI Search service (empty if service does not exist)')
output principalId string = aiSearchExists && aiSearchEnabled && !empty(aiSearchName) ? reference(resourceId('Microsoft.Search/searchServices', safeName), '2024-06-01-preview', 'Full').identity.principalId : ''

@description('AI Search service name')
output name string = aiSearchName

@description('AI Search exists flag')
output exists bool = aiSearchExists && aiSearchEnabled && !empty(aiSearchName)
