...
output dashboardInsightsExists bool = length(resourceNames.dashboardInsights) > 0 ? checkResourceExists[3].outputs.exists : false
output applicationInsightExists bool = length(resourceNames.applicationInsight) > 0 ? checkResourceExists[4].outputs.exists : false
output aiServicesExists bool = length(resourceNames.aiServices) > 0 ? checkResourceExists[5].outputs.exists : false
output bingExists bool = length(resourceNames.bing) > 0 ? checkResourceExists[6].outputs.exists : false
output containerAppsEnvExists bool = length(resourceNames.containerAppsEnv) > 0 ? checkResourceExists[7].outputs.exists : false
...
output functionAppExists bool = length(resourceNames.functionApp) > 0 ? checkResourceExists[11].outputs.exists : false
output webAppExists bool = length(resourceNames.webApp) > 0 ? checkResourceExists[12].outputs.exists : false
output funcAppServicePlanExists bool = length(resourceNames.funcAppServicePlan) > 0 ? checkResourceExists[13].outputs.exists : false
output webbAppServicePlanExists bool = length(resourceNames.webbAppServicePlan) > 0 ? checkResourceExists[14].outputs.exists : false
output keyvaultExists bool = length(resourceNames.keyvault) > 0 ? checkResourceExists[15].outputs.exists : false
output miACAExists bool = length(resourceNames.miACA) > 0 ? checkResourceExists[16].outputs.exists : false
output miPrjExists bool = length(resourceNames.miPrj) > 0 ? checkResourceExists[17].outputs.exists : false
output storageAccount1001Exists bool = length(resourceNames.storageAccount1001) > 0 ? checkResourceExists[18].outputs.exists : false
output storageAccount2001Exists bool = length(resourceNames.storageAccount2001) > 0 ? checkResourceExists[19].outputs.exists : false
output redisExists bool = length(resourceNames.redis) > 0 ? checkResourceExists[20].outputs.exists : false
output postgreSQLExists bool = length(resourceNames.postgreSQL) > 0 ? checkResourceExists[21].outputs.exists : false
output sqlServerExists bool = length(resourceNames.sqlServer) > 0 ? checkResourceExists[22].outputs.exists : false
output sqlDatabaseExists bool = length(resourceNames.sqlDatabase) > 0 ? checkResourceExists[23].outputs.exists : false
