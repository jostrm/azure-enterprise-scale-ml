...
param scope string = subscription().id // tenantResourceId('Microsoft.Management/managementGroups', topLevelManagementGroupPrefix)

@description('Also deploy the optional unused-resource cost optimization initiative. The private-DNS bootstrap leaves this disabled.')
param includeCostOptimization bool = false
...
var deploymentLocation = '"location": "${location}"'
...
var processPolicySetDefinitionsAll = [for content in loadPolicySetDefinitions.All: replace(replace(content, templateVars.scope, scope), templateVars.defaultDeploymentLocation, deploymentLocation)]
var processPolicySetDefinitionsAzureCloud = [for content in loadPolicySetDefinitions.AzureCloud: replace(replace(content, templateVars.scope, scope), templateVars.defaultDeploymentLocation, deploymentLocation)]
