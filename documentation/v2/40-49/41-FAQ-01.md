...
## Q: How can I troubleshoot errors in ADO/GHA pipeline to create AIFactory project?
...
## Q: ADO/GHA pipelines - Why can't it read from the seeding key vault when running pipelines?

- A: See the [Seeding key vault - troubleshooting](../10-19/12-seeding-keyvault.md)

## ADO pipeline failed, log message says something about *rbacDBX2AazureMLwithProjectSP001sdcde*?
...
```
- A: This is because the ApplicationID was passed instead of the ObjectID when Bicep tried to set RBAC. The error likely occurred because the ApplicationID or wrong ObjectID was copied from Microsoft Entra ID to the seeding key vault for the secret name *esml-project001-sp-oid* in the seeding key vault.
- How to fix: 
    - 1) Go to the [seeding key vault](../10-19/12-seeding-keyvault.md) and verify that the VALUE of the secret name that ends with *-oid* such as *esml-project001-sp-oid* is correct.
...
### Q: Networking: How do I troubleshoot Azure Machine Learning networking? Diagnostics? Troubleshoot private endpoint connection problems? 
...
## CORS - LNA: Foundry: Web browser: EDGE, CHROME
To have Foundry working with private endpoints, from a laptop or AVD, creating agents without "AXIOS Error", you need to allow - LNA
