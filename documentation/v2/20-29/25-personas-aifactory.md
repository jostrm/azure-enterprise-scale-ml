...
The IaC acceleration in Enterprise Scale AI Factory, creates a baseline of personas, to have more granualar access within the `project team`  and within `core team`.
These personas is `across services`, and `across environments`.
...
Built-in Azure roles `Project team` members has on resource group level:
- **Contributor**
- ****AcrPush**
