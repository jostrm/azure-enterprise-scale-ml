...
**Continuation status (21 September 2026): prepare-only.** Fresh, retained v1.25 reviews for project001 and project003 in Sweden/Dev001 are both blocked by missing Azure DevOps pipeline binding and Azure Blob lock enrollment. A stale local published-release cache was repaired without changing the consumer checkout or working files. No confirmation was submitted, no new job appeared, and the register stayed unchanged. These reviews do not establish deployment readiness or complete the film. See [the producer readiness update](#21-september-review-only-readiness-update).
...
**21 September capture supplement:** new, actual maximized-window images are saved under `image-screenshots\maui-take2`. The accepted monitor-work-area captures are **3840 x 2076**: `take2-review-20260921-09-project003-setup-clean.png`, `take2-review-20260921-10-readiness-clean.png`, and `take2-review-20260921-11-pipeline-guidance-clean.png`. The adjacent `take2-review-20260921-captures.json` records provenance and preserved superseded takes. These illustrate the new setup/recovery navigation only; they are not deployment footage and have not replaced the Part 1 video or deck. The full deployment film and its final PowerPoint remain pending.

**Still to record:** the real project001 Function update, project003 deployment, and corresponding deployment-execution CLI/API workflows. Part 1 does not claim those operations occurred. Project001's Function remains absent and project003's canonical resource group was absent in the latest exact-scope readiness evidence; a recorded configuration save is not a substitute for deployment.

**Continuation (18 September):** the owner explicitly approved Germany reusing the existing Sweden subscription. The following native actions are now **saved and recorded**, in order: the Germany West Central factory with release **1.25**, Dev002 and **`172.17.0.0/20`**, capacity **one**; project004 placed only in Germany Dev002; and a configuration-only reference clone with new identities, no projects and no copied resource ownership. The original Sweden configuration and the clone's Germany source remain unchanged. Five actual authenticated API/CLI read or help commands follow the UX chapters. No project workload deployment is claimed. Keep actual subscription/tenant IDs in private operator material, not this public guide.

Germany's new range does not overlap the registered Sweden ranges or the VNets observed in the supplied subscription; this is not an organizational IPAM or peering approval. The blueprint deliberately retains Germany's CIDR, so it is a saved reference configuration, **not** an isolated Azure environment. The shared subscription is not described as dedicated-subscription isolation.
...
| Implemented in the 21 September review-navigation build | Pinned factory/environment/project context and **Set up project deployment** next to the selected scope | The shortcut requires an existing matching placement and explicitly includes that project. It opens setup and highlights the release section; it does not prepare, confirm, save, or run anything. The generic action is now **Review deployment**, with an explicit project/common choice. |
| Implemented in the 21 September review-navigation build | Missing-binding reviews offer **Open pipeline setup** | The link opens the correct form and uses the existing blue pulse on the writer field. Unsaved binding inputs remain intact. It neither fabricates enrollment nor provisions locks, identity, or a runner. |
...
| Implemented in the clipboard-fixed continuation build | A busy Windows clipboard no longer crashes **Copy key for Swagger** | The specific clipboard-busy error receives bounded retries, then a recoverable explanation without exposing the key. Other errors are not silently swallowed. |
...
### 21 September review-only readiness update

The active instruction is **prepare reviews only**. Historical cost and infrastructure approvals below are context, not authorization to confirm, provision enrollment, edit remote pipelines, or start jobs during this pass.

| Scope | Latest observed result |
|---|---|
| project001 / Dev001 / 1.25 | Runtime preview: `can_execute=false`, one blocker, zero warnings. Existing `enableFunction` is the string `"true"`; no new configuration change was needed. |
| project003 / Dev001 / 1.25 | Runtime preview: `can_execute=false`, one blocker, zero warnings. Existing draft has only a Dev001 placement. |
| Exact blocker, both targets | `Configure the factory's pipeline binding ado.json with a reviewed remote and operator-provisioned Azure Blob lock enrollment before deployment.` |
| Saved binding | The register has no saved binding for the selected route; its generated `ado.json` projection is also absent. Do not create that projection by hand: register-backed changes must use the reviewed binding API/form. |
| Release-cache repair | The live published `release/v1.25` commit was missing from the orange submodule's object database. It was fetched from the existing local purple object database with no checkout, merge, reset, or remote write. Fresh reviews passed that gate and reached the binding gate. This is not an installed-factory upgrade. |
| Non-deletion evidence | The settings endpoint does **not** expose the deletion/cleanup flags. Separately inspected saved project `variables.json` projections showed `cleanFoundryCaphost`, `debugEnableCleaning`, `deleteAllForProject`, `deleteAllServicesForProject`, `deleteKeyvaultAlso`, and `enableDeleteForDisabledResources` all `"false"` in both sections for both projects. A later executable review must still validate the authoritative frozen inputs and what-if; missing fields are not proof of false. |
| Retained evidence | Both blocked SDK receipts and before/after job/register evidence are retained in the private production session, outside the public repository. Receipts are short-lived; archived copies are evidence, not reusable execution approval. |

Next prerequisite: the operator's exact ADO repository/ref, writer identity, Linux execution runner, writable resource-group scopes, shared dependencies, and matching live Blob-lock enrollment must be reviewed. A manually fabricated binding, name-based ownership adoption, or a dependency-only common scope is not a substitute. Subsequent gates may still fail after binding setup; they have not been passed in this review.

The previous CLI troubleshooting did not demonstrate an API/CLI incompatibility: catalog success returns the catalog object directly, without an `ok` wrapper, and folder arguments must identify an absolute server-local path. Handle command exit status before interpreting JSON. Use the app's explicit key-copy control, keep the key process-local, and clear the copied clipboard value without overwriting later user clipboard content.

| Gate | Current state / required decision |
|---|---|
...
| Execution enrollment | **ADO authentication is working**, and the user identified control of the target environment. The additional Linux ADO worker and exact lock-container/role setup are now approved; do not ask for those approvals again. The existing Windows VM/agent and legacy pipelines remain unchanged. Canonical enrollment still requires verified project001 ownership, an independently enrolled common writer and actual coordination of all overlapping writers. Merely identifying the administrator or adding a runner does not establish lease participation. The full v1.25 project path includes common/network/RBAC writes, which cannot be executed through a dependency-only common scope. Do not silently adopt common resources or bypass the guard. |
| Consent and cost | Approval covers project001 DEV update and project003 costs, the separately described additional Linux ADO worker, exact private lock container with a container-scoped role for the existing identity, verified project001 ownership reconciliation, exact project003 Dev RG creation, and intentional Germany subscription reuse. It is not blanket approval for unrelated RBAC, common/hub adoption, remote pipeline replacement or deletion. Saved defaults include Foundry/capability host, Search, AML, Cosmos DB, ADF, Databricks, Event Hubs, Bot, and admin VM. “Defaults” is not a low-cost guarantee. |
...
| Phase 2 result | Region belongs to the **factory**, not the scale set. Intentional subscription reuse is approved. `ai-spider-germany`, prefix `spider-gw-`, Dev002, release 1.25, ADO, `172.17.0.0/20`, capacity one, and project004 in Dev002 only are saved. Sweden is unchanged. These are configuration results; no Germany Azure deployment is claimed. Own-subscription mode does not create subscriptions or make a reused subscription exclusive. |
| Phase 3 result | `ai-spider-blueprint`, prefix `spider-ref-`, is saved as an explicitly **undeployed configuration reference** cloned from Germany with release 1.25 and no projects. New UUIDs and cleared ownership were verified, and the source is unchanged. The copied subscription/CIDR remain; the overlap warning was reviewed. Do not deploy this copied network or promise an unimplemented retarget flow. |
...
| Release evidence | No project workload deployment success is established. The recorded regional and clone results are real local saves, not cloud jobs. Before using a project-deployment success line, retain the real job result and scoped Azure evidence privately. Local helper repairs must be released through a supported patched contract before being represented as the published runtime; do not silently change the pinned v1.25 source. |
...
**Director / clicks / pulse:** Only after the producer gate passes and execution is separately authorized, select project001 and Dev001, then choose **Set up project deployment**. The project is explicitly included; the pinned context remains visible while the blue pulse reveals release setup. Select 1.25, choose **Prepare review**, and inspect the resolved ref, target, costs, non-deletion checks, and allowed changes. Keep consent and warnings visible. Pulse the separate confirmation control, accept the exact scope, confirm once, and follow its real job/terminal. Under the current prepare-only instruction, stop before acceptance/confirmation and keep this unexecuted scene out of the completed-film claim.
...
**Director / clicks / pulse:** Select the existing project003 UUID and Dev001, choose **Set up project deployment**, and prepare its own runtime review. Inspect the complete review, then perform confirmation only when separately authorized. Capture the actual job and scoped resulting resource group/service inventory. Never reuse project001's receipt. This execution scene remains pending under the current prepare-only boundary.
...
**Teacher:** “Germany West Central is a new regional factory identity in our existing Azure Factory register. Region belongs to the factory; the scale set inherits it. I select Germany West Central, choose 1.25, and use the approved subscription and nonoverlapping network for Development suffix 002. This demonstration intentionally reuses Sweden's subscription; the regional identity and network are separate, not the subscription.”
...
**Recorded outcome:** Germany and project004 were saved through their separate native confirmations. Use the saved definition evidence, not a recreated write for another take.
...
**Director / clicks / pulse:** Open **Advanced → Clone configuration**, select Germany as the source and the reference prefix `spider-ref-`, explicitly choose 1.25, and set **include projects: none**. Prepare; inspect the retained subscription and CIDR values, warnings, reset ownership, and new identities. Confirm only this **undeployed configuration reference** and reload its saved result. This exact save is already recorded; do not repeat it.
...
**Evidence / cut:** Verify new UUIDs and unchanged source privately, and absence of a runtime job for this operation. Show the saved configuration-only result, never a fictional Azure clone. The recorded example has no project definitions or copied resource ownership.
...
This last line is a limited presentation summary, not proof of backend enrollment or deployment. The settings API can omit deletion/cleanup fields; the scan above detects exposed enabled flags but cannot certify missing flags as false. Inspect the saved project configuration and require the backend's authoritative frozen-input and non-deletion checks before any execution approval. Review settings privately; do not print `$settings`, the complete catalog, environment variables, or secrets.
