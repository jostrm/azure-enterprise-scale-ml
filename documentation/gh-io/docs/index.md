...
!!! note
    This project uses Azure CLI, Bicep, and GitHub Actions or Azure DevOps pipelines.
    Choose the supported entrypoint and authentication setup for your selected release.

## Configure, create and update

- Keep one project's complete **`variables.json`** with `dev` and `stage_prod`
  settings, including the three environment subscriptions. YAML and `.env` routes
  remain supported. See [Configuration and API](parameters/index.md).
- Create a scale set with **`ADO-create-new-aifactory-scaleset.sh`** or
  **`GHA-create-new-aifactory-scaleset.sh`**.
- Update/run a project with **`ADO-update-aifactory-and-run-project.sh`** or
  **`GH-update-aifactory-and-run-project.sh`** (`GHA-...` is an alias).
- Integrate a trusted ITSM/portal runner through the **API layer**:
  validate, prepare, approve, execute and track the exact job. Preparation alone
  does not deploy.

Use the [complete variable reference](parameters/advanced.md) for current
JSON/YAML/GitHub names, defaults and conditional requirements.
...
| Resource | Link |
|---|---|
| Setup Guide | [Configuration and API](parameters/index.md), [GitHub Actions](orchestrators/gh.md), [Azure DevOps](orchestrators/ado.md) |
