...
class IESMLPipelineStepMap:
    ...
    def get_train_map(self, dataset_folder_names):
        ...
        step1 = 'in2silver_{}.{}'.format(dataset_folder_names[0],self._compute_type_dbx)
        step2 = 'in2silver_{}.{}'.format(dataset_folder_names[1],self._compute_type_py)
        step4 = 'silver_merged_2_gold.{}'.format(self._compute_type_dbx)
        step5 = 'train_split_and_register.{}'.format(self._compute_type_dbx)
        step6 = 'train_manual.{}'.format(self._compute_type_dbx)

        self.IN_2_GOLD_TRAIN_notebook_mapping= [
           #{'code': nb1,'compute_type':self._compute_type_dbx,'date_folder_or': None,'compute_name':'s-p002-aml-rt91','cluster_id':'0912-204847-wimps924'}, # IN_2_SILVER note: date_folder_override: Showcase static lookup data. Overrides main date_folder, which all other steps reads from
           #{'code': script,'compute_type':self._compute_type_py, 'date_folder_or': '2021-01-01 10:35:01.243860'},
           #{'code': nb4,'compute_type':self._compute_type_dbx, 'date_folder_or': None,'compute_name':'s-p002-aml-rt91','cluster_id':'0912-204847-wimps924'}, # MERGE 2 GOLD
           #{'code': nb6,'compute_type':self._compute_type_dbx,'date_folder_or': None, 'compute_name':'s-p002-aml-rt91','cluster_id':'0912-204847-wimps924'} # TRAIN & Register model
        ]
    ...
    def get_inference_map(self, dataset_folder_names):
        ...
        step1 = 'in2silver_{}.{}'.format(dataset_folder_names[0],self._compute_type_dbx)
        step2 = 'in2silver_{}.{}'.format(dataset_folder_names[1],self._compute_type_dbx)
        step4 = 'silver_merged_2_gold.{}'.format(self._compute_type_dbx)
