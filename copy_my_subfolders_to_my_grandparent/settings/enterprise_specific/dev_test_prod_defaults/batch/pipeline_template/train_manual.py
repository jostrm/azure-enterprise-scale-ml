...
def init():
    ...
    try:
        ...
        validate_ds = next(iter(run.input_datasets.items()))[1] # Get 2nd DATASET: GOLD_VALIDATE
        #validate_df = validate_ds.to_pandas_dataframe()

        test_ds = next(iter(run.input_datasets.items()))[1] # Get 3rd DATASET: GOLD_TEST
        ...
        esml_scoring_date_out = date_infolder.strftime('%Y/%m/%d')
