...
- `Enterprise "cockpit"` over ALL your projects & models. 
 ...
 - `Enterprise Datalake, with ADLS GEN2, and logical DataMesh`: Not physical DataMesh, since we want to avoid scattered Blob/GEN2 storage accounts with myriad/different security & micro services.
  - ESML DataMesh avoids `The curse of security diversity`: Some have vNet & private endpoints..some have not. Different RBAC models on same data, multiple copies on MASTER DATA..event-sync nightmare)
...
- `Azure Devops templates (2 purposes)`: 
    - purpose 1) MLOps (CI/CD): `INNER loop` and `OUTER loop` (across dev,test,prod workspaces). 
    - purpose 2) To kick off BICEP release pipeline, to proviosion the `AI Factory & ESMLProject` automatically (PM "order a ESMLProject" in 15min)
- `ESML DATALAKE`: Battle tested design supports DataMesh,IoT,ML concepts such as MLOps, BRONZE, SILVER, GOLD
